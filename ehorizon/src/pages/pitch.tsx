...
const THEME_OPTIONS = [
  ...
  { value: "sustain", label: "Sustainability" },
];
...
export default function PitchRegister() {
  ...
  const validate = () => {
    ...
    if (abstractFile && abstractFile.type !== "application/pdf") return "Abstract must be a PDF file.";
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("members", JSON.stringify(members)); // matches your single-model JSONField

      // 🔁 Replace with your Django API
      const res = await fetch("http://127.0.0.1:8000/api/pitch/register", {
        method: "POST",
        body: formData,
      });
